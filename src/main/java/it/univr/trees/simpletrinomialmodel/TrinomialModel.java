package it.univr.trees.simpletrinomialmodel;
...
public class TrinomialModel {
	...
	/**
	 * It constructs an object representing a Trinomial model.
	 * 
	...
	private void generateValues() {
		...
		for (int numberOfTimeStepsAhead = 1; numberOfTimeStepsAhead < numberOfTimes; numberOfTimeStepsAhead++) {
			for (int exponentOfUp = 0; exponentOfUp <= 2*numberOfTimeStepsAhead; exponentOfUp++) {
				...
				/*
				 * First we fill the last position in the vector, which has all downs, and then we proceed until the
				 * last position, which has all ups
				 */
	...
	/**
	 * It returns all the possible values of the trinomial model at the given time index.
	 ...
	 * the given time index. The entries of the array are placed in decreasing order of the underlying 
	 ...
	 * 		   The entries of the array are placed in decreasing order of the underlying tree model
	...
	public double[] getTransformedValuesAtGivenTimeIndex(int timeIndex, DoubleUnaryOperator transformFunction) {
		//the possible values of the trinomial model
	 ...
	 * values of (possibly a function of) a trinomial model at time timeIndex+1. 
	 * 
	 * @param trinomialValues, values of (possibly a function of) a trinomial model at time timeIndex+1
	 * @param timeIndex, the time index
	 * @return the array of the discounted conditional expectations at timeIndex of trinomialValues. 
	 */
